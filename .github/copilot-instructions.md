...
- **Live consent records are sensitive:** `relicensing#25` (formerly `cbioportal#12282`), `relicensing#24` (formerly `cbioportal#12271`), and `docs/relicensing/contributor-consent-status.md` are live records. Preserve recorded consents exactly; make only surgical edits when needed, and avoid broad rewrites that could alter or obscure existing consent entries.
- **Current live entries:** Nikolaus Schultz is already recorded in `relicensing#25` and JJ Gao is already recorded in `relicensing#24`. Preserve those consent entries exactly in any future edits.
